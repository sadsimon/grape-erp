...
<script lang="ts" setup name="MaDataTreeLeft2">
...
const filterNode = (name: string, data: any) => {
	...
	// 动态获取 label 属性的值
	const labelProp = prop.props?.label || 'name' // 如果没有传入 props 或 label，使用默认值 'name'
	return data[labelProp].includes(name)
}
...
.ma-left-tree {
	min-height: calc(100vh - 600px - var(--theme-header-height));
	overflow: hidden;
	white-space: nowrap;
