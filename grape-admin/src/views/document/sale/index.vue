		...
		<el-container class="layout-container">
			<el-card class="head layout-query">
				<el-form ref="saveRef" :inline="true" 	 :rules="dataRules" :model="dataForm">
					<div style="font-size:20px; text-align:center;margin-bottom: 15px;">销售出库单</div>
					...
					<el-form-item prop="contactunitsId" label="购买单位">
						<GrContactunitsInput width="250px" @select="updateBalance" :disabled="isfinish" :type="'1'" :title="'购买单位'" v-model="dataForm.contactunitsId"></GrContactunitsInput>
					</el-form-item>
						...
						<el-form ref="dataFormRef" style="margin-top: 15px;" :model="dataForm" :rules="dataRules" label-width="100px">
							<el-row :gutter="5">
								<el-col :span="24" :lg="24" :md="24" :sm="24">
									<el-form-item prop="advanceIn" label="预收款">
										<GrNumberInput
...
<script setup lang="ts" name="DocumentSaleIndex">
	...
	const updateBalance = async ()=>{
		if(dataForm.value.contactunitsId){
			...
			balance.value = calcChain(contactunitBalance.value).sub(dataForm.value.advanceIn).toNumber()
		}
	}
	...
	const shouldAmount = computed(() => {
		return (calcChain(finalAmount.value.toString()).sub(dataForm.value.documentAccountDetailList.reduce((sum, item) => {
