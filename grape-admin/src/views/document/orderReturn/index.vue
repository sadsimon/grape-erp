		...
		<el-container class="layout-container">
			<el-card class="head layout-query">
				<el-form ref="saveRef" :inline="true" 	 :rules="dataRules" :model="dataForm">
					<div style="font-size:20px; text-align:center;margin-bottom: 15px;">采购退货单</div>
			...
			<GrDocumentFoot @isArrowUp="isArrowUpFu" maxHeight="300" height="150">
				<el-tabs v-model="activeName" type="card">
						...
						<el-form ref="dataFormRef" style="margin-top: 15px;" :model="dataForm" :rules="dataRules" label-width="100px">
							<el-row :gutter="5">
								<el-col :span="24" :lg="24" :md="24" :sm="24">
									<el-form-item prop="advanceOut" label="退预付款">
										<GrNumberInput
					...
					<el-tab-pane label="采购费用" name="settlement">
						待开发
					</el-tab-pane>
				</el-tabs>
			</GrDocumentFoot>
...
<script setup lang="ts" name="DocumentOrderReturnIndex">
	...
	const updateBalance = async ()=>{
		if(dataForm.value.contactunitsId){
			...
			balance.value = calcChain(contactunitBalance.value).add(dataForm.value.advanceOut).toNumber()
		}
	}
	...
	const shouldAmount = computed(() => {
		...
		}, new Big(0)).toNumber()).add(dataForm.value.advanceOut).sub(finalAmount.value.toString()).toNumber() )
	})
