...
import { useProductCategoryListApi, useProductCategorySubmitApi } from '@/api/product/category'
...
const init = (isUpdate: boolean, row: any) => {
	...
	// 存在则为修改
	if (row) {
		getProductCategory(isUpdate, row)
	} else {
		dataForm.pid = ''
...
const getProductCategoryList = async () => {
	const res = await useProductCategoryListApi()
	productCategoryList.value = res.data
}

// 获取信息
const getProductCategory = (isUpdate: boolean, row: any) => {
